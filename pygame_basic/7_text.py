...
# 적 enemy 캐릭터
enemy = pygame.image.load("C:/Users/Junje Ahn/Desktop/Python-project1/pygame_basic/enemy.png")
enemy_size = enemy.get_rect().size # 이미지의 크기를 구해옴
enemy_width = enemy_size[0] # 캐릭터의 가로 크기
enemy_height = enemy_size[1] # 캐릭터의 세로 크기
enemy_x_pos = screen_width / 2 - enemy_width / 2 # 화면 가로의 절반에 위치
enemy_y_pos = screen_height / 2 - enemy_height / 2 # 화면의 제일 아래에 위치
...
# 캐릭터가 1초 동안 100만큼 이동을 해야된다.
# 10 fps : 1초 동안에 10번 동작 -> 1번에 10만큼 이동해야된다.
# 20 fps : 1초 동안에 20번 동작 -> 1번에 5만큼 이동해야된다.
    ...
    for event in pygame.event.get(): # 생성되는 이벤트 (클릭이나 마우스 움직임, 키보드 입력 등등)
        ...
        if event.type == pygame.KEYDOWN: # 키가 눌러졌는지 확임
            if event.key == pygame.K_LEFT: # 캐릭터를 왼쪽으로
                to_x -= character_speed # character_speed만큼 왼쪽으로 (to_x = to_x - character_speed)
    ...
    character_x_pos += to_x * dt # dt를 곱하는 이유는 속도는 일정하게하고 fps값을 바꾸기 위함
    character_y_pos += to_y * dt # while문이라서 계속 누적이 되는 값. to_x,y를 0으로 초기화한다 해도 돌아오지 X
    ...
    # 타이머 집어 넣기
    # 경과 시간 계산
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 # 경과 시간을 초단위로 표시. 현재시간에서 시작시간을 뺌.
...
# 잠시 대기
pygame.time.delay(2000) # 2초 대기하다가 종료
